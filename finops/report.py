...
def build_report(baseline_usd: float, optimized_usd: float, levers: dict,
                 sustainability: dict | None = None, extensions: dict | None = None, period: str = "hàng tháng") -> str:
    """Tạo báo cáo tối ưu hóa chi phí GPU bằng Tiếng Việt."""
    savings = baseline_usd - optimized_usd
    pct = (savings / baseline_usd * 100.0) if baseline_usd > 0 else 0.0
    lines = [
        "# NimbusAI — Báo cáo Tối ưu hóa Chi phí GPU (GPU FinOps Report)",
        "",
        f"**Kỳ báo cáo:** Tháng 6/2026 (Tính theo {period})  ",
        f"**Chi phí Ban đầu (Baseline Spend):** ${baseline_usd:,.0f}  ",
        f"**Chi phí Sau Tối ưu (Optimized Spend):** ${optimized_usd:,.0f}  ",
        f"**Tổng Chi phí Tiết kiệm (Projected savings):** ${savings:,.0f}  (**{pct:.0f}%**)",
    ...
    for name, amount in levers.items():
        lever_pct = (amount / baseline_usd * 100.0) if baseline_usd > 0 else 0.0
        lines.append(f"| {name} | ${amount:,.0f} | {lever_pct:.1f}% |")
    ...
    if sustainability:
        lines += [
            ...
            "",
            f"- **Mức tiêu thụ năng lượng bình quân**: {sustainability.get('wh_per_query', 0):.2f} Wh / truy vấn.",
            f"- **Lượng phát thải Carbon bình quân**: {sustainability.get('carbon_g', 0):.3f} gCO2e / truy vấn (tại vùng `us-east-1`).",
            f"- **Khu vực tối ưu nhất (Sạch nhất + Rẻ nhất)**: `{sustainability.get('best_region', 'europe-north1')}` (Sử dụng 100% năng lượng tái tạo/thủy điện với mức carbon cực thấp).",
        ]
    ...
    if extensions:
        lines += [
            ...
            f"- **Ngưỡng Kinh tế của Prompt Caching**: Xác định điểm hòa vốn khi tỷ lệ đọc cache (Cache Read Hit Rate) đạt tối thiểu **{extensions.get('cache_threshold', '20%')}**, giúp đảm bảo chi phí tiết kiệm vượt qua chi phí lưu trữ/ghi overhead.",
            ...
            f"- **Lập lịch Nhận thức Carbon (Carbon-Aware Scheduling)**: Chuyển toàn bộ job training gián đoạn sang vùng `europe-north1` giúp giảm **{extensions.get('carbon_savings_pct', '80.0')}%** tổng lượng carbon phát thải.",
        ]

    lines += ["", "---", "_Báo cáo được tổng hợp tự động từ dữ liệu telemetry tháng 6/2026._"]
    return "\n".join(lines)
